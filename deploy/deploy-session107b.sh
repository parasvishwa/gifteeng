#!/usr/bin/env bash
# session107b — Retry hero banner text overlay
# Ships the missing auth-b2b permissions infrastructure (permissions.ts,
# permissions.guard.ts, team controllers) alongside the hero-banner
# schema migration + admin/web changes.

set -euo pipefail
cd /srv/gifteeng
set -a; . /srv/gifteeng/.env; set +a

tar xzf /tmp/patch_session107b.tar.gz
