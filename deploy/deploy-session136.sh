#!/usr/bin/env bash
# session136 — marketplace Phase 3: buyer-side seller selection
#   seller-offers API, SellerPicker on PDP, sellerProductId threaded through
#   cart + checkout + order items.
set -euo pipefail
cd /srv/gifteeng
set -a; . /srv/gifteeng/.env; set +a

tar xzf /tmp/patch_session136.tar.gz
...
# Build shared first — the API + web import @gifteeng/shared from dist/
pnpm --filter @gifteeng/shared build 2>&1 | tail -3
