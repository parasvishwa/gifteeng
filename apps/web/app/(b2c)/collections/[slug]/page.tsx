...
export async function generateMetadata({
  ...
  const description =
    meta?.description ?? `Browse the ${name} collection at Gifteeng — handpicked personalised gifts.`;
  ...
  return {
    ...
    openGraph: {
      title:       `${name} – Personalised Gift Collection | Gifteeng`,
      description,
      url,
      type:        "website",
    },
...
export default async function CollectionPage({
  ...
  // ── JSON-LD: BreadcrumbList + ItemList for the collection ────────────────
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name,                item: `${SITE}/collections/${slug}` },
    ],
  };
