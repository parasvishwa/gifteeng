...
async function fetchProducts(params: {
  search?: string; category?: string; collection?: string; sort?: string;
  ...
  // Collection filter — matches Collection.slug via ProductCollection join.
  // Enables `/products?collection=birthday-gifts` deep-links from the
  // admin Collections page or any marketing CTA.
  ...
  try {
    ...
    const items = rawItems.map((p) =>
      normaliseListProduct({
        ...
        ratingAvg: typeof (p as any).ratingAvg === "number" ? (p as any).ratingAvg : undefined,
        reviewCount: typeof (p as any).reviewCount === "number" ? (p as any).reviewCount : undefined,
        _count: (p as unknown as { _count?: { variantOptions?: number; orderItems?: number } })._count,
      }),
    );
...
// ── Product Cards ─────────────────────────────────────────────────────────────
// Grid view uses the shared ProductCard component for a unified look across
// all sections. List view keeps its own compact row layout.
...
function ProductListCard({ product, wishlisted, onWishlist }: {
  ...
  return (
    <div className="group relative flex gap-4 rounded-2xl bg-card overflow-hidden transition-all duration-200 hover:shadow-lg hover:shadow-black/5">
      <Link href={`/products/${product.slug}`} className="flex gap-4 flex-1 p-4">
...
function ProductsPageInner() {
  ...
  // Collection-slug filter. Set by /super-admin/collections deep-links and
  // any marketing CTA that wants to land on "/products?collection=...".
  ...
  // Refs mirror state so the IntersectionObserver can check the latest values
  // without re-mounting on every render. The observer was previously being
  // re-created on each products.length / loadingMore / currentPage change,
  // which caused the initial intersection callback to refire and the
  // "Loading more…" row to flicker as duplicate loads stacked.
  ...
  }, [urlSearch, urlCategory, urlCollection, urlSort, urlMinPrice, urlMaxPrice, urlCustomizable, urlTag]);
  ...
  const handleLoadMore = async () => {
    // Hard guard: if a load is already in flight, bail. The observer guard
    // (loadingMoreRef) covers the common case, but rapid intersections can
    // sneak in between setLoadingMore(true) committing and the next render.
    if (inFlightRef.current) return;
    // Don't fire if we've already loaded everything
    ...
    try {
      ...
      // Always sync total so hasMore reflects server truth
      setTotal(data.total);
      // If the server returned nothing, mark as exhausted regardless of total
      if (data.items.length === 0) {
        setTotal(0); // force hasMore → false
      } else {
        const filtered = applyClientFilters(data.items, {
          isCustomizable: urlCustomizable || undefined,
          minPrice: urlMinPrice || undefined,
  ...
  // Keep the latest closure reachable from the (stable) observer callback.
  handleLoadMoreRef.current = handleLoadMore;

  // Mirror loading state into refs so the (stable) observer callback below
  // always sees the latest values without us having to re-create the observer.
  ...
  // The observer is created ONCE per mount. Re-creating it on every state
  // change made the initial-intersection callback refire on each remount,
  // which stacked rapid duplicate page loads and produced a visible flicker
  // on the "Loading more…" row.
  useEffect(() => {
  ...
  if (urlCollection)
    activeFilters.push({
      key: "collection",
      label: `Collection · ${urlCollection}`,
      clear: () => pushUrl({ collection: "" }),
    });
  if (urlSort) {
                  ...
                  <>
                    {viewMode === "grid" && (
                      <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-4">
                        {products.map((p) => {
                          const base = p.basePrice !== undefined ? parseFloat(String(p.basePrice)) : 0;
                          return (
                            <ProductCard
                              key={p.id}
                              name={p.title}
                              ...
                              onToggleWish={(e) => { e.stopPropagation(); toggleWishlist(p.id); }}
                            />
                          );
                        })}
                      </div>
                    ...
                    <div className="mt-10 flex flex-col items-center gap-3">
                      ...
                      {/* Sentinel — IntersectionObserver triggers auto-load.
                          We render the spinner persistently whenever hasMore
                          so the row never pops in/out between page loads.
                          When idle (between loads) the spinner dims slightly
                          via opacity transition so it doesn't feel deceptive. */}
                      {hasMore && (
                        <div ref={sentinelRef} className="w-full h-10 flex items-center justify-center">
                          <div
                            className={`flex items-center gap-2 text-sm text-muted-foreground transition-opacity duration-200 ${
