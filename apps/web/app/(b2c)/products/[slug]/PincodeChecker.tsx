        ...
        <div className="mt-3 space-y-2.5">
          {result.deliverable ? (
            <>
              ...
              {/* Dispatch + delivery + COD + returns — single inline row.
                  Previously this was a 2-column card grid PLUS two pill
                  chips below, eating ~140px of vertical space. Compressed
                  to one wrapped row of small text so the entire pincode
                  block stays under 90px on a typical width. */}
              {result.dispatchDate && result.deliveryDate && (
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
                  <span className="inline-flex items-center gap-1">
                    <Package className="w-3 h-3" />
                    Ships by <span className="font-bold text-foreground">{prettyDate(result.dispatchDate)}</span>
                  </span>
                  <span className="text-border">·</span>
                  <span className="inline-flex items-center gap-1">
                    <Truck className="w-3 h-3" />
                    Arrives <span className="font-bold text-foreground">{prettyDate(result.deliveryDate)}</span>
                  </span>
                  <span className="text-border">·</span>
                  <span
                  ...
                  <span className="text-border">·</span>
                  <span className="inline-flex items-center gap-1 font-bold text-sky-700 dark:text-sky-400">
