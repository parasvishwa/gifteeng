...
export default function PlayPage() {
  ...
  useEffect(() => {
    ...
    if (!token) { router.push("/auth?next=/play"); return; }
    (async () => {
      try {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#1A040A" }}>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#1A040A" }}>
        <div className="text-center text-white/60 p-6">
          ...
          <button onClick={() => window.location.reload()} className="mt-4 px-5 py-2 rounded-full bg-white/10 hover:bg-white/15 active:scale-[0.97] text-white text-sm [transition:background-color_160ms_cubic-bezier(0.23,1,0.32,1),transform_160ms_cubic-bezier(0.23,1,0.32,1)]">Retry</button>
        </div>
      </div>
    );
  }
  ...
  return (
    // "Gift Casino" hero uses the same celebrative-maroon recipe as the
    // homepage Return Gifts and Coins banners so the dark theming reads as
    // one identity instead of three different darks. Deep wine gradient
    // base + brand-pink halo top-left + amber halo bottom-right.
    <div
      className="relative min-h-screen overflow-hidden"
      style={{
        background:
          "radial-gradient(ellipse 45% 40% at 12% 8%, rgba(239,55,82,0.22) 0%, transparent 70%)," +
          "radial-gradient(ellipse 40% 35% at 92% 95%, rgba(255,180,80,0.12) 0%, transparent 70%)," +
          "linear-gradient(160deg, #3D0814 0%, #2A0610 40%, #1A040A 70%, #120308 100%)",
      }}
    >
      ...
      {/* Glittery sparkle confetti — gold + white + brand-pink dots scattered
          across the casino canvas. Same palette as the homepage banners so
          the celebration cue carries through. Density bumped to 14 since the
          casino canvas is taller (full viewport). */}
      <div aria-hidden className="absolute inset-0 pointer-events-none overflow-hidden">
        {[
          { top: "4%",  left: "6%",  size: 3, color: "#fbbf24", opacity: 0.65 },
          { top: "9%",  left: "48%", size: 2, color: "#ffffff", opacity: 0.45 },
          { top: "14%", left: "82%", size: 4, color: "#fbbf24", opacity: 0.55 },
          { top: "22%", left: "22%", size: 2, color: "#ffffff", opacity: 0.4  },
          { top: "31%", left: "68%", size: 3, color: "#EF3752", opacity: 0.5  },
          { top: "38%", left: "12%", size: 2, color: "#fbbf24", opacity: 0.55 },
          { top: "46%", left: "92%", size: 4, color: "#ffffff", opacity: 0.4  },
          { top: "55%", left: "38%", size: 3, color: "#fbbf24", opacity: 0.5  },
          { top: "63%", left: "75%", size: 2, color: "#EF3752", opacity: 0.45 },
          { top: "72%", left: "8%",  size: 3, color: "#fbbf24", opacity: 0.5  },
          { top: "80%", left: "55%", size: 2, color: "#ffffff", opacity: 0.4  },
          { top: "87%", left: "32%", size: 4, color: "#fbbf24", opacity: 0.45 },
          { top: "93%", left: "88%", size: 2, color: "#EF3752", opacity: 0.5  },
          { top: "97%", left: "18%", size: 3, color: "#ffffff", opacity: 0.4  },
        ].map((d, i) => (
          <span
            key={i}
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-6 md:py-10">
        ...
        <div className="flex items-center justify-between mb-5 md:mb-7">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="w-9 h-9 rounded-full bg-white/5 hover:bg-white/10 active:scale-[0.9] flex items-center justify-center [transition:background-color_160ms_cubic-bezier(0.23,1,0.32,1),transform_160ms_cubic-bezier(0.23,1,0.32,1)]"
            ...
            <Link
              ...
              className="flex items-center gap-1.5 px-3 h-9 rounded-full bg-white/5 hover:bg-white/10 active:scale-[0.95] [transition:background-color_160ms_cubic-bezier(0.23,1,0.32,1),transform_160ms_cubic-bezier(0.23,1,0.32,1)]"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-5 md:mb-7">
          ...
          <HeroCard accent="#f59e0b">
            <div className="flex items-center gap-3">
              <div className="relative shrink-0">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl bg-[#3D0F1A]"
                >
          ...
          <Link
            href="/album"
            className="group relative block rounded-2xl overflow-hidden border border-white/10 p-4 md:p-5 [transition:transform_220ms_cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-1 hover:scale-[1.02] active:scale-[0.97] bg-[#2A0810]"
          >
            <div className="flex items-center gap-3 mb-3">
              <div
                className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 bg-[#3D0F1A]"
              >
                📖
              </div>
              <div className="flex-1 min-w-0">
              ...
              <ChevronRight className="w-4 h-4 text-white/40 group-hover:text-white group-hover:translate-x-0.5 [transition:transform_160ms_cubic-bezier(0.23,1,0.32,1),color_160ms_cubic-bezier(0.23,1,0.32,1)] shrink-0" />
            </div>
        ...
        {/* ─── YOUR REWARDS ─── */}
        {rewards.length > 0 && (
          <div className="rounded-2xl border border-white/10 p-4 md:p-5 mb-6 md:mb-7 bg-[#2A0810]"
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/cart" className="text-xs text-[#EF3752] hover:opacity-80 flex items-center gap-1 font-bold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {rewards.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className="relative rounded-2xl p-3 border overflow-hidden [transition:transform_200ms_cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-0.5 active:scale-[0.98] bg-[#2A0810]"
                    style={{
                      borderColor: isApplied ? "rgba(239,55,82,0.35)" : "rgba(255,255,255,0.1)",
                      borderTop: isApplied ? "2px solid #EF3752" : undefined,
                    }}
                  >
                    <div className="relative flex items-center gap-3">
                      <div
                        className="w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0 bg-[#3D0F1A]"
                      >
                        {rewardIcon(r.type)}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-[10px] text-white/40">
                          {r.code && (
                            <button
                              onClick={() => copy(r.code)}
                              className="font-mono font-bold text-white/70 hover:text-white active:scale-[0.95] flex items-center gap-1 [transition:color_150ms_cubic-bezier(0.23,1,0.32,1),transform_150ms_cubic-bezier(0.23,1,0.32,1)]"
                            >
                              {r.code}
...
function HeroCard({ children, accent }: { children: React.ReactNode; accent: string }) {
  return (
    <div
      className="relative rounded-2xl overflow-hidden border border-white/10 p-4 md:p-5 bg-[#2A0810]"
...
function GameCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={!available}
      className="group relative aspect-[3/4] rounded-2xl overflow-hidden [transition:transform_200ms_cubic-bezier(0.23,1,0.32,1),box-shadow_200ms_cubic-bezier(0.23,1,0.32,1),opacity_200ms_ease] hover:-translate-y-1.5 hover:scale-[1.03] active:scale-[0.97] disabled:cursor-not-allowed will-change-transform bg-[#2A0810] border border-white/10"
      style={{
    ...
    >
      ...
      <div className="relative h-full flex flex-col p-3 md:p-3.5">
        ...
        {/* CTA pill */}
        <div
          className="w-full py-1.5 rounded-xl text-center text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] [transition:transform_160ms_cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.04] text-white"
          style={{
...
function SeasonalBanner() {
  ...
  return (
    <div
      className="rounded-2xl p-3 md:p-4 mb-5 md:mb-6 flex items-center gap-3"
      style={{
        background: `linear-gradient(135deg, ${active.accent}22 0%, rgba(42,8,16,0.95) 60%)`,
