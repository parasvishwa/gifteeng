...
export async function generateMetadata({
  ...
  if (!q?.trim()) {
    return {
      title: "Search Personalized Gifts | Gifteeng",
      description:
        "Search for personalized gifts by occasion, recipient or product type on Gifteeng. Custom photo frames, mugs, keychains & more.",
      robots: { index: false, follow: true },
    };
  }

  const trimQ = q.trim();
  const cap   = trimQ.charAt(0).toUpperCase() + trimQ.slice(1);
  return {
    title:       `${cap} Gifts — Search Results | Gifteeng`,
    description: `Find personalized ${trimQ} gifts on Gifteeng — custom options starting ₹99 with free India-wide delivery.`,
    alternates:  { canonical: `${SITE}/search?q=${encodeURIComponent(trimQ)}` },
    openGraph: {
      title:       `${cap} gifts | Gifteeng`,
      description: `Personalized ${trimQ} gifts from ₹99 — Gifteeng`,
      url:         `${SITE}/search?q=${encodeURIComponent(trimQ)}`,
      type:        "website",
    },
    // Only index searches with a meaningful query (3+ chars) to avoid thin pages
    robots: { index: trimQ.length >= 3, follow: true },
  };
}
