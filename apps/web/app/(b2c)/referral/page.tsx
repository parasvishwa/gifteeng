...
export default function ReferralPage() {
  ...
  const token = typeof window !== "undefined" ? getB2cToken() : null;

  useEffect(() => {
    const tok = getB2cToken();
    if (!tok) { setLoading(false); return; }
    fetch(`${API_BASE_URL}/api/referrals/me`, {
  ...
  const totalEarned = info?.total_earned ?? usesCount * 50;
  const availableCredits = info?.available_credits ?? usesCount * 50;
  ...
  const shareText = referralCode
    ? `Hey! Use my referral code ${referralCode} on Gifteeng and get ₹50 off your first order! 🎁 https://gifteeng.com?ref=${referralCode}`
    : "";
      ...
      <div className="pt-16 md:pt-20">
        <div className="container mx-auto px-4 pt-20 md:pt-24 pb-24 md:pb-12 max-w-2xl">
          ...
          <div className="bg-card rounded-2xl p-8 md:p-12 shadow-sm text-center mb-8">
            ...
            {loading ? (
              <div className="animate-pulse bg-muted rounded-xl p-4 max-w-xs mx-auto h-14 mb-6" />
            ) : referralCode ? (
              <>
              ...
              <p className="text-xs text-muted-foreground mb-6">No active code yet. Please contact support.</p>
            )}

            <div className="flex flex-col sm:flex-row justify-center gap-3">
              <a
                href={shareText ? `https://wa.me/?text=${encodeURIComponent(shareText)}` : "#"}
                target="_blank"
                rel="noopener noreferrer"
                className={`bg-[#EF3752] text-white font-bold text-sm px-6 py-3 flex items-center justify-center gap-2 rounded-xl hover:opacity-90 ${!referralCode ? "opacity-40 pointer-events-none" : ""}`}
              >
              ...
              <button
                ...
                className="bg-muted border border-border text-foreground font-bold text-sm px-6 py-3 flex items-center justify-center gap-2 rounded-xl hover:bg-accent transition-colors disabled:opacity-40"
              >
                {copiedLink ? <CheckCircle className="w-4 h-4 text-green-500" /> : <Share2 className="w-4 h-4" />}
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-card rounded-2xl p-5 shadow-sm text-center">
              <p className="text-2xl font-display font-bold text-foreground">₹{totalEarned}</p>
              <p className="text-[10px] text-muted-foreground">Total Earned</p>
            </div>
            <div className="bg-card rounded-2xl p-5 shadow-sm text-center">
              <p className="text-2xl font-display font-bold text-foreground">₹{availableCredits}</p>
