...
//
// Layout: HOME · SHOP · [GIFT CASINO hero ↑] · CART · ME
...
const MobileBottomNav = () => {
  ...
  const isHome   = pathname === "/b2c" || pathname === "/b2c/";
  const isShop   = shopOpen || (!isHome && pathname.startsWith("/b2c/products"));
  const isCasino = pathname === "/b2c/play";
  const isCart   = pathname === "/b2c/cart";
  const isMe     = pathname === "/b2c/account";
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50">
        ...
        >
          <div className="flex items-stretch" style={{ height: 58 }}>
            ...
            <HeroTab
              active={isCasino}
              onClick={() => { setShopOpen(false); router.push("/b2c/play"); }}
            />
            ...
            <RegularTab
              ...
              active={isCart}
              badge={totalItems}
              onClick={() => router.push("/b2c/cart")}
            />
            <RegularTab
              icon={User}
              label="ME"
              active={isMe}
              onClick={() => router.push("/b2c/account")}
            />
          </div>
      ...
      <Sheet open={shopOpen} onOpenChange={(o) => { setShopOpen(o); if (!o) setShopTab("categories"); }}>
        <SheetContent side="bottom" className="rounded-t-3xl max-h-[80vh] flex flex-col pb-0">
          ...
          <div className="shrink-0 grid grid-cols-4 gap-2 mt-3 mb-3">
            {[
              { label: "All",       path: "/b2c/products",              emoji: "🛍️" },
              { label: "Top",       path: "/b2c/products?sort=popular", emoji: "⭐" },
              { label: "New",       path: "/b2c/products?sort=newest",  emoji: "✨" },
              { label: "Quiz",      path: "/b2c/gift-quiz",             emoji: "🎁" },
            ].map((q) => (
              <button
                key={q.label}
                ...
                {categoryTree.length === 0 ? (
                  ...
                  categoryTree.map((cat) => (
                    <div key={cat.id} className="rounded-xl overflow-hidden">
                      <button
                        onClick={() => navTo(`/b2c/products?category=${encodeURIComponent(cat.name)}`)}
                        ...
                        <div className="grid grid-cols-2 gap-x-1 gap-y-0.5 pl-12 pb-1 pr-2">
                          {cat.subcategories.map((sub) => (
                            <button
                              key={sub.id}
                              onClick={() =>
                                navTo(`/b2c/products?category=${encodeURIComponent(sub.name)}`)
                              }
                      ...
                      <div className="grid grid-cols-2 gap-0.5">
                        {cols.map((c) => {
                          ...
                          return (
                            <button
                              key={c.id}
                              onClick={() => navTo(`/b2c/collections/${encodeURIComponent(slug)}`)}
