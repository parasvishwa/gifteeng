...
export default function FloatingSpinWheel() {
  ...
  useEffect(() => {
    // Show only to logged-in B2C users
    const token = getB2cToken();
    setEnabled(!!token);
  }, []);
  ...
  return (
    <>
      <button
        onClick={() => router.push("/b2c/play")}
        className="fixed bottom-24 md:bottom-8 left-4 z-50 group flex flex-col items-center gap-1"
      ...
      >
        ...
        {/* Card */}
        <span className="relative flex flex-col items-center justify-center gap-1 w-16 rounded-2xl shadow-2xl group-hover:scale-105 transition-transform overflow-hidden py-2.5 px-1"
          style={{
            background: "linear-gradient(145deg,#1a0533 0%,#3b0764 60%,#6d28d9 100%)",
            border: "1.5px solid rgba(167,139,250,0.35)",
            boxShadow: "0 8px 32px rgba(109,40,217,0.5), 0 0 0 1px rgba(255,255,255,0.06) inset",
          }}
        >
          {/* Shimmer bar */}
          ...
          {/* Label */}
          <span className="text-white font-black text-[9px] tracking-widest uppercase leading-none">Gift</span>
          <span className="text-violet-300 font-bold text-[8px] tracking-widest uppercase leading-none">Casino</span>
        </span>
