...
function buildSpec(kind: BadgeKind, inventory?: number): BadgeSpec {
  switch (kind) {
    case "soldout":
      return { kind, emoji: "🚫", label: "SOLD OUT", bg: "#374151", fg: "#F9FAFB", ring: "#1F2937", priority: 0 };
    case "low_stock":
      return { kind, emoji: "⚠️", label: `Only ${inventory ?? 0} left`, bg: "#FEF2F2", fg: "#DC2626", ring: "#FCA5A5", priority: 1 };
    case "trending":
      return { kind, emoji: "🔥", label: "TRENDING", bg: "#FFF7ED", fg: "#C2410C", ring: "#FED7AA", priority: 2 };
    case "bestseller":
      return { kind, emoji: "🏆", label: "BEST SELLER", bg: "#FEF3C7", fg: "#92400E", ring: "#FDE68A", priority: 3 };
    case "featured":
      return { kind, emoji: "⭐", label: "FEATURED", bg: "#FEF3C7", fg: "#A16207", ring: "#FDE68A", priority: 4 };
    case "new":
      return { kind, emoji: "✨", label: "NEW", bg: "#ECFDF5", fg: "#047857", ring: "#A7F3D0", priority: 5 };
    case "customizable":
      return { kind, emoji: "✏️", label: "CUSTOMIZABLE", bg: "#F5F3FF", fg: "#6D28D9", ring: "#DDD6FE", priority: 6 };
  }
}
    ...
    >
      {badges.map((b) => (
        <span key={b.kind} style={chipStyle(b, size)}>
          <span aria-hidden style={{ fontSize: size === "card" ? 10 : 13, lineHeight: 1 }}>
            {b.emoji}
          </span>
          <span>{b.label}</span>
        </span>
