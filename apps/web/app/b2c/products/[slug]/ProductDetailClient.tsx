  ...
  function OptionBtn({ opt, isMobile }: { opt: VariantOpt; isMobile: boolean }) {
    ...
    return (
      <button
        ...
        className={`group relative flex flex-col items-center gap-0.5 rounded-md border-2 p-0.5 transition-all focus:outline-none w-full ${
          active
            ? "border-[#EF3752] scale-[1.03]"
            : "border-border hover:border-[#EF3752]/50"
        }`}
      >
        ...
        <span className={`text-center leading-tight truncate w-full px-0.5 py-0.5 ${isMobile ? "text-[10px]" : "text-[11px]"} ${active ? "font-semibold text-[#EF3752]" : "text-foreground/80"}`}>
          {opt.value}
          {opt.priceDelta ? (
...
export function ProductDetailClient({ product }: { product: Product }) {
  ...
  // Fire product_view analytics once per mount.
  useEffect(() => {
  ...
  const onAdd = useCallback(async () => {
    ...
    try {
      ...
      await addItem({
        productId: product.id,
        title: product.title,
        priceLabel: `${currencySymbol}${totalPrice}`,
        quantity: qty,
        image: cartImage,
        slug: product.slug,
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-md border border-border bg-card px-3 py-2">
          ...
          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-orange-600 dark:text-orange-400">
            <Flame className="w-3 h-3" fill="currentColor" /> Trending gift
          </span>
        </div>
        ...
        <div className="rounded-2xl bg-card p-5">
          <div className="flex items-end flex-wrap gap-x-3 gap-y-1">
            <p className="font-display text-3xl md:text-4xl font-black leading-none text-foreground tabular-nums">
          ...
          {selectedDeltas.length > 0 && (
            <p className="mt-1.5 text-[11px] text-muted-foreground">
              Base {currencySymbol}{basePrice.toLocaleString("en-IN")}
              {selectedDeltas.map((d) => (
                <span key={d.name}>
                  {" "}+ {currencySymbol}{d.delta} ({d.value})
                </span>
              ))}
            </p>
          )}
        ...
        {product.isCustomizable === true ? (
          ...
          <button
            ...
            className="w-full rounded-xl px-5 py-4 text-base font-black text-white bg-[#EF3752] disabled:opacity-50 transition-all active:scale-[0.98] shadow-lg shadow-[#EF3752]/20 flex items-center justify-center gap-2"
          >
            {allVariantsOos || inventory === 0
              ? "Out of Stock"
              : <><Wand2 className="w-5 h-5" />Personalise &amp; Add to Cart</>}
          </button>
        ) : (
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              disabled={ctaDisabled}
              onClick={onAdd}
              className="py-2.5 rounded-xl text-sm font-black bg-muted border border-border text-foreground disabled:opacity-50 transition-all active:scale-[0.98]"
            >
              {adding ? "Adding…" : (allVariantsOos || inventory === 0) ? "Out of Stock" : <><ShoppingCart className="w-4 h-4 mr-1.5 inline-block" />Add to Cart</>}
            </button>
            <button
              type="button"
              disabled={ctaDisabled}
              onClick={async () => {
                await onAdd();
                setTimeout(() => router.push("/b2c/checkout"), 400);
              }}
              className="py-2.5 rounded-xl text-sm font-black text-white bg-[#EF3752] disabled:opacity-50 transition-all active:scale-[0.98]"
            >
              {(allVariantsOos || inventory === 0) ? "Notify Me" : <><Zap className="w-4 h-4 mr-1.5 inline-block" />Buy Now</>}
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          {[
            { emoji: "🚚", label: "Free delivery", sub: "On orders ₹999+" },
        ...
        {/* ── How to personalise (3-step flow) ────────────────────────── */}
        {product.isCustomizable && (
          <div className="rounded-xl border border-[#EF3752]/20 bg-[#EF3752]/[0.04] p-4">
            <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#EF3752] mb-3">
              How personalisation works
            </p>
            <div className="flex items-stretch gap-0">
              {[
                { n: "1", label: "Add Name", sub: "Type the name or message" },
                { n: "2", label: "Preview", sub: "See it live before ordering" },
                { n: "3", label: "Order", sub: "Delivered in 5–7 days" },
              ].map((s, i) => (
                <div key={s.n} className="flex items-center flex-1 min-w-0">
                  <div className="flex flex-col items-center text-center gap-1 flex-1 min-w-0 px-1">
                    <div className="w-8 h-8 rounded-full bg-[#EF3752] text-white text-sm font-black flex items-center justify-center shrink-0">
                      {s.n}
                    </div>
                  ...
                  {i < 2 && (
                    <span className="text-[#EF3752]/40 text-base font-black shrink-0 self-start mt-2">›</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="mx-auto flex max-w-lg items-center gap-2">
          ...
          {product.isCustomizable === true ? (
            <button
              ...
              disabled={ctaDisabled}
              onClick={() => router.push(`/customize/${product.slug}`)}
              className="shrink-0 flex-1 px-4 py-2.5 rounded-xl text-[11px] font-black text-white bg-[#EF3752] disabled:opacity-50 flex items-center justify-center gap-1"
            >
              <Wand2 className="w-3.5 h-3.5" />Customize Now →
            </button>
          ) : (
            <>
              <button
              ...
              <button
                ...
                className="shrink-0 px-4 py-2.5 rounded-xl text-[11px] font-black text-white bg-[#EF3752] disabled:opacity-50"
              >
                Buy Now
              </button>
            </>
